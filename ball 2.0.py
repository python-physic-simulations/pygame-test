...
pygame.display.set_caption('ball 2.0 BETA')
...
class Ball():
    def __init__(self, x, y, radius= 10, color = RED, speed = vector(0.1,0.1),\
        ...
        self.mass = mass
        self.acc = acc
        self.friction=self.mass*0.0002 #g=1
    ...
    def update_pos(self,time_diff):
        ...
        if int(self.baricenter.y + self.radius) >= screen_height :
            ...
            self.baricenter.y=screen_height-self.radius
        if int(self.baricenter.x + self.radius) > screen_width :
            self.speed.x = - self.speed.x
        if int(self.baricenter.x - self.radius) < 0 :
            self.speed.x = - self.speed.x 
    def update_speed(self, time_diff):
        self.speed = self.speed + self.acc * time_diff
ball1 = Ball(50, screen_height-250 , speed = vector(0.5,0), acc = vector(0,0.001))
ball1.draw(screen_surface)
clock= pygame.time.Clock()
while True:
    time_diff = clock.get_time()
    screen_surface.fill(WHITE)
    ball1.update_speed(time_diff)
